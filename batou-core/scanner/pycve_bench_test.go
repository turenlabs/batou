...
// pycveExpected describes the expected finding for one Python CVE fixture.
// Loaded from expected.json in each CVE subdirectory. Same shape as
// gocveExpected in gocve_bench_test.go — kept as a separate type so the
// two corpora can evolve independently without forcing a shared struct.
type pycveExpected struct {
	CVE         string `json:"cve"`
...
// pycveResult holds the scan outcome for one Python CVE fixture. Mirrors
// gocveResult so readers familiar with the Go scorecard JSON do not need
// to learn a second schema.
type pycveResult struct {
	CVE                      string   `json:"cve"`
	Category                 string   `json:"category"`
	CWE                      string   `json:"cwe"`
	Description              string   `json:"description"`
...
// scanPyTree walks a directory and scans every .py file via
...
func scanPyTree(t *testing.T, treeRoot, scanWorkDir string) (firedCWEs, firedRules, blockingCWEs []string) {
	t.Helper()
...
// loadPyCVEFixture reads expected.json from a CVE subdirectory.
func loadPyCVEFixture(dir string) (pycveExpected, error) {
	var exp pycveExpected
...
// TestPyCVEBench runs Batou over the curated Python CVE corpus at
// testdata/pycve-bench/ and emits a scorecard. Same shape as TestGoCVEBench:
...
// the scorecard goes to t.Log and testdata/pycve-bench/results.json.
//
// Two scorecards are emitted side-by-side, mirroring the Go harness:
...
//                  expected CWE per the curated cweAncestors table shared
//                  with the Go corpus (no fork).
func TestPyCVEBench(t *testing.T) {
	root := projectRoot()
	corpusDir := filepath.Join(root, "testdata", "pycve-bench")

	entries, err := os.ReadDir(corpusDir)
	if err != nil {
		t.Skipf("Python CVE corpus not found at %s: %v", corpusDir, err)
		return
	}
	...
	if len(fixtures) == 0 {
		t.Skip("no CVE fixtures found under testdata/pycve-bench/CVE-*")
		return
	}

	t.Logf("Loaded %d Python CVE fixtures from %s", len(fixtures), corpusDir)

	benchDir := t.TempDir()

	var results []pycveResult
	var blockDets []cveBlockDetection

	for _, fixDir := range fixtures {
		exp, err := loadPyCVEFixture(fixDir)
		if err != nil {
		...
		vulnCWEs, vulnRules, vulnBlockCWEs := scanPyTree(t, vulnDir, vulnWork)
		safeCWEs, safeRules, safeBlockCWEs := scanPyTree(t, safeDir, safeWork)
		...
		results = append(results, pycveResult{
			CVE:                      exp.CVE,
			Category:                 exp.Category,
			CWE:                      exp.CWE,
	...
	// --- Scorecard (same layout as TestGoCVEBench) ---
	t.Logf("\n=== PyCVE Benchmark Scorecard ===")
	...
	blockLane := logCVEBlockLane(t, "PyCVE", blockDets)

	// Persist results JSON with both scorecards (same struct shape as
	// gocve-bench/results.json so any downstream tooling can read both).
	outDir := filepath.Join(root, "testdata", "pycve-bench")
	...
	type benchOutput struct {
		...
		Results    []pycveResult      `json:"results"`
	}
	out := benchOutput{
		Cases:  len(results),
		TPRate: tpRate,
