...
// javacveExpected describes the expected finding for one Java CVE fixture.
...
// javacveResult holds the scan outcome for one Java CVE fixture. Mirrors
// jscveResult so readers familiar with the JS scorecard JSON do not need
// to learn a second schema.
type javacveResult struct {
	CVE                      string   `json:"cve"`
	Category                 string   `json:"category"`
	CWE                      string   `json:"cwe"`
	Description              string   `json:"description"`
...
// scanJavaTree walks a directory and scans every .java file via
...
func scanJavaTree(t *testing.T, treeRoot, scanWorkDir string) (firedCWEs, firedRules, blockingCWEs []string) {
	t.Helper()
...
// loadJavaCVEFixture reads expected.json from a fixture subdirectory.
func loadJavaCVEFixture(dir string) (javacveExpected, error) {
	var exp javacveExpected
...
// TestJavaCVEBench runs Batou over the curated Java CVE corpus at
// testdata/javacve-bench/ and emits a scorecard. Same shape as
// TestJsCVEBench: each fixture subdirectory has vuln/ and safe/ trees
...
// testdata/javacve-bench/results.json.
//
...
func TestJavaCVEBench(t *testing.T) {
	root := projectRoot()
	corpusDir := filepath.Join(root, "testdata", "javacve-bench")

	entries, err := os.ReadDir(corpusDir)
	if err != nil {
		t.Skipf("Java CVE corpus not found at %s: %v", corpusDir, err)
		return
	}
	...
	if len(fixtures) == 0 {
		t.Skip("no fixtures found under testdata/javacve-bench/{CVE-*,classic-*}")
		return
	}

	t.Logf("Loaded %d Java CVE fixtures from %s", len(fixtures), corpusDir)

	benchDir := t.TempDir()

	var results []javacveResult
	var blockDets []cveBlockDetection

	for _, fixDir := range fixtures {
		exp, err := loadJavaCVEFixture(fixDir)
		if err != nil {
		...
		vulnCWEs, vulnRules, vulnBlockCWEs := scanJavaTree(t, vulnDir, vulnWork)
		safeCWEs, safeRules, safeBlockCWEs := scanJavaTree(t, safeDir, safeWork)
		...
		results = append(results, javacveResult{
			CVE:                      exp.CVE,
			Category:                 exp.Category,
			CWE:                      exp.CWE,
	...
	// --- Scorecard (same layout as TestJsCVEBench) ---
	t.Logf("\n=== JavaCVE Benchmark Scorecard ===")
	...
	blockLane := logCVEBlockLane(t, "JavaCVE", blockDets)

	// Persist results JSON with both scorecards (same struct shape as
	// jscve-bench/results.json so downstream tooling can read both).
	outDir := filepath.Join(root, "testdata", "javacve-bench")
	...
	type benchOutput struct {
		...
		Results    []javacveResult    `json:"results"`
	}
	out := benchOutput{
		Cases:  len(results),
		TPRate: tpRate,
