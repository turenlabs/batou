...
// rubycveExpected describes the expected finding for one Ruby CVE fixture.
// Loaded from expected.json in each fixture subdirectory. Same shape as
// javacveExpected / pycveExpected — kept as a separate type so the
// corpora can evolve independently without forcing a shared struct.
type rubycveExpected struct {
	CVE         string `json:"cve"`
...
// rubycveResult holds the scan outcome for one Ruby CVE fixture. Mirrors
// javacveResult so readers familiar with the Java scorecard JSON do not
// need to learn a second schema.
type rubycveResult struct {
	CVE                      string   `json:"cve"`
	Category                 string   `json:"category"`
	CWE                      string   `json:"cwe"`
	Description              string   `json:"description"`
...
// scanRubyTree walks a directory and scans every .rb file via
// testutil.ScanContentInDir, returning the union of fired CWEs and rule
// IDs, plus the subset of fired CWEs that came from BLOCKING findings
// (RiskScore ≥ 0.7) for the dual-lane block scorecard. The non-test scan
// path prefix (/app/...) avoids the scanner's isTestFile short-circuit —
// the fixtures look enough like real controllers that we want them treated
// as production code.
func scanRubyTree(t *testing.T, treeRoot, scanWorkDir string) (firedCWEs, firedRules, blockingCWEs []string) {
	t.Helper()
...
// loadRubyCVEFixture reads expected.json from a fixture subdirectory.
func loadRubyCVEFixture(dir string) (rubycveExpected, error) {
	var exp rubycveExpected
...
// TestRubyCVEBench runs Batou over the curated Ruby CVE corpus at
// testdata/rubycve-bench/ and emits a scorecard. Same shape as
// TestJavaCVEBench: each fixture subdirectory has vuln/ and safe/ trees
// plus expected.json. The test does NOT assert specific TP/FP counts —
// it is a measurement tool; the scorecard goes to t.Log and
// testdata/rubycve-bench/results.json.
//
...
//
// Two scorecards are emitted side-by-side, mirroring the Java, Python
// and JS harnesses:
...
//     Python, JS and Java corpora (no fork).
func TestRubyCVEBench(t *testing.T) {
	root := projectRoot()
	corpusDir := filepath.Join(root, "testdata", "rubycve-bench")

	entries, err := os.ReadDir(corpusDir)
	if err != nil {
		t.Skipf("Ruby CVE corpus not found at %s: %v", corpusDir, err)
		return
	}
	...
	if len(fixtures) == 0 {
		t.Skip("no fixtures found under testdata/rubycve-bench/{CVE-*,classic-*}")
		return
	}

	t.Logf("Loaded %d Ruby CVE fixtures from %s", len(fixtures), corpusDir)

	benchDir := t.TempDir()

	var results []rubycveResult
	var blockDets []cveBlockDetection

	for _, fixDir := range fixtures {
		exp, err := loadRubyCVEFixture(fixDir)
		if err != nil {
		...
		vulnCWEs, vulnRules, vulnBlockCWEs := scanRubyTree(t, vulnDir, vulnWork)
		safeCWEs, safeRules, safeBlockCWEs := scanRubyTree(t, safeDir, safeWork)
		...
		results = append(results, rubycveResult{
			CVE:                      exp.CVE,
			Category:                 exp.Category,
			CWE:                      exp.CWE,
	...
	// --- Scorecard (same layout as TestJavaCVEBench) ---
	t.Logf("\n=== RubyCVE Benchmark Scorecard ===")
	...
	blockLane := logCVEBlockLane(t, "RubyCVE", blockDets)

	// Persist results JSON with both scorecards (same struct shape as
	// javacve-bench/results.json so downstream tooling can read both).
	outDir := filepath.Join(root, "testdata", "rubycve-bench")
	...
	type benchOutput struct {
		...
		Results    []rubycveResult    `json:"results"`
	}
	out := benchOutput{
		Cases:  len(results),
		TPRate: tpRate,
