...
// jscveExpected describes the expected finding for one JS/TS CVE fixture.
// Loaded from expected.json in each CVE subdirectory. Same shape as
// pycveExpected — kept as a separate type so the two corpora can evolve
// independently without forcing a shared struct.
type jscveExpected struct {
	CVE         string `json:"cve"`
...
// jscveResult holds the scan outcome for one JS/TS CVE fixture. Mirrors
// pycveResult so readers familiar with the Python scorecard JSON do not
// need to learn a second schema.
type jscveResult struct {
	CVE                      string   `json:"cve"`
	Category                 string   `json:"category"`
	CWE                      string   `json:"cwe"`
	Description              string   `json:"description"`
...
// scanJsTree walks a directory and scans every .js / .ts / .jsx / .tsx
// file via testutil.ScanContent, returning the union of fired CWEs and
// rule IDs, plus the subset of fired CWEs that came from BLOCKING findings
// (RiskScore ≥ 0.7) for the dual-lane block scorecard.
//
...
func scanJsTree(t *testing.T, treeRoot, scanWorkDir string) (firedCWEs, firedRules, blockingCWEs []string) {
	t.Helper()
...
// loadJsCVEFixture reads expected.json from a CVE subdirectory.
func loadJsCVEFixture(dir string) (jscveExpected, error) {
	var exp jscveExpected
...
// TestJsCVEBench runs Batou over the curated JS/TS CVE corpus at
// testdata/jscve-bench/ and emits a scorecard. Same shape as TestPyCVEBench:
// each CVE subdirectory has vuln/ and safe/ trees plus expected.json. The
// test does NOT assert specific TP/FP counts — it is a measurement tool;
// the scorecard goes to t.Log and testdata/jscve-bench/results.json.
//
// Two scorecards are emitted side-by-side, mirroring the Go and Python
// harnesses:
...
//     and Python corpora (no fork).
func TestJsCVEBench(t *testing.T) {
	root := projectRoot()
	corpusDir := filepath.Join(root, "testdata", "jscve-bench")

	entries, err := os.ReadDir(corpusDir)
	if err != nil {
		t.Skipf("JS/TS CVE corpus not found at %s: %v", corpusDir, err)
		return
	}
	...
	if len(fixtures) == 0 {
		t.Skip("no CVE fixtures found under testdata/jscve-bench/CVE-*")
		return
	}

	t.Logf("Loaded %d JS/TS CVE fixtures from %s", len(fixtures), corpusDir)

	benchDir := t.TempDir()

	var results []jscveResult
	var blockDets []cveBlockDetection

	for _, fixDir := range fixtures {
		exp, err := loadJsCVEFixture(fixDir)
		if err != nil {
		...
		vulnCWEs, vulnRules, vulnBlockCWEs := scanJsTree(t, vulnDir, vulnWork)
		safeCWEs, safeRules, safeBlockCWEs := scanJsTree(t, safeDir, safeWork)
		...
		results = append(results, jscveResult{
			CVE:                      exp.CVE,
			Category:                 exp.Category,
			CWE:                      exp.CWE,
	...
	// --- Scorecard (same layout as TestPyCVEBench) ---
	t.Logf("\n=== JsCVE Benchmark Scorecard ===")
	...
	blockLane := logCVEBlockLane(t, "JsCVE", blockDets)

	// Persist results JSON with both scorecards (same struct shape as
	// pycve-bench/results.json so downstream tooling can read both).
	outDir := filepath.Join(root, "testdata", "jscve-bench")
	...
	type benchOutput struct {
		...
		Results    []jscveResult      `json:"results"`
	}
	out := benchOutput{
		Cases:  len(results),
		TPRate: tpRate,
