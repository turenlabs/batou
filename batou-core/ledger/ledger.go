...
type Entry struct {
	...
	// SuppressedReasons is index-aligned with SuppressedRules: entry i holds
	// the `-- reason` text of the batou:ignore directive that suppressed rule
	// i ("" when the directive carried no reason). Omitted entirely when no
	// suppressed finding stated a reason.
...
func Record(sessionID string, result *reporter.ScanResult) error {
	...
	// Collect suppressed rule IDs (and the per-directive `-- reason` each
	// suppression stated, index-aligned) for the audit log.
	var suppressedRules, suppressedReasons []string
	var anyReason bool
