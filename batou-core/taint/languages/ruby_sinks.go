...
func (rubyCatalog) Sinks() []taint.SinkDef {
	return []taint.SinkDef{
		...
		{ID: "ruby.rails.render.file", Category: taint.SnkFileRead, Language: rules.LangRuby, Pattern: `render\s+file\s*:`, ObjectType: "ActionController", MethodName: "render file:", DangerousArgs: []int{0}, Severity: rules.High, Description: "Rails render file: with tainted path (arbitrary file read, CVE-2019-5418)", CWEID: "CWE-22", OWASPCategory: "A01:2021-Broken Access Control"},
		...
		// --- File operations (CWE-22) ---
		{
			...
			Severity:      rules.High,
			Description:   "File move with potentially tainted source or destination path",
			CWEID:         "CWE-22",
