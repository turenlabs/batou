#!/usr/bin/env python3
"""Populate the Go CVE benchmark corpus from a manifest.

Reads tools/gocve_manifest.yaml (or a path passed on the command line) and
materialises one fixture directory per CVE under testdata/gocve-bench/:

    testdata/gocve-bench/CVE-2024-24786/
        vuln/...        # vulnerable Go sources
        safe/...        # patched Go sources
        expected.json   # CWE + metadata for the harness

Two manifest modes are supported:

* mode: stage  -- copy curated reproducers from tools/gocve_fixtures/<CVE>/.
                  This is the default mode used by the in-tree corpus
                  because reduced reproducers are easier to audit than
                  cloning multi-MB upstream repos.

* mode: clone  -- shallow-clone `repo` at `vuln_commit`/`fixed_commit` and
                  copy the listed `files[]` into the vuln and safe trees
                  respectively. Skipped (with a warning) when git is not
                  on PATH or when network access fails.

The harness in batou-core/scanner/gocve_bench_test.go does not need the
script to run — fixtures committed under testdata/gocve-bench/ are scanned
...
DEFAULT_MANIFEST = REPO_ROOT / "tools" / "gocve_manifest.yaml"
CORPUS_DIR = REPO_ROOT / "testdata" / "gocve-bench"
...
def _load_yaml(path: Path) -> list[dict[str, Any]]:
    ...
    Supported:
      - Lines starting with "- key: value" begin a new entry.
      - Subsequent "  key: value" lines belong to the current entry.
      - "  key: >-" begins a folded block; following lines indented
        further are joined with single spaces, blank lines split paragraphs.
    ...
    for raw in text.splitlines():
        # Drop comments unless inside folded block (folded blocks shouldn't
        # contain "#" at line start in our manifest, so this is safe).
        if folding_key is None:
            stripped_no_comment = raw.split("#", 1)[0].rstrip()
        else:
            # In folded block: a less-indented line ends the block.
...
def _clone_entry(entry: dict[str, Any]) -> tuple[int, int]:
    ...
    n_vuln = n_safe = 0
    with tempfile.TemporaryDirectory(prefix=f"gocve-{cve}-") as tmp:
