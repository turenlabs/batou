// Safe: Grails GORM with parameterized HQL queries
...
class UserService {
    ...
    def findSingle(String email) {
        User.find("from User where email = :email", [email: email])
    }
    ...
    def deleteByName(String name) {
        User.executeUpdate("delete from User where name = :name", [name: name])
    }
