...
func (r *SwiftURLSessionNoSSL) Description() string {
	return "Detects Swift URLSession delegates that unconditionally accept server trust challenges, disabling SSL/TLS certificate validation."
}
...
func (r *SwiftURLSessionNoSSL) Scan(ctx *rules.ScanContext) []rules.Finding {
	if !rules.GMatchFile(reURLSessionNoValidate, ctx) {
		return nil
	}
	...
	for i, line := range lines {
		...
		if m := rules.GFindLower(reDispositionUseCredential, line, lowered[i]); m != "" {
			matched = m
		...
		} else if m := rules.GFindLower(reURLCredentialTrust, line, lowered[i]); m != "" {
			matched = m
		...
		} else if m := rules.GFindLower(reCompletionUseCredential, line, lowered[i]); m != "" {
			matched = m
...
func (r *SwiftKeychainNoAuth) Description() string {
	return "Detects Swift Keychain items stored with kSecAttrAccessibleAlways or without access control flags, making them readable without device authentication."
}
...
func (r *SwiftKeychainNoAuth) Scan(ctx *rules.ScanContext) []rules.Finding {
	if !rules.GMatchFile(reKeychainAdd, ctx) && !rules.GMatchFile(reKeychainQuery, ctx) {
		return nil
	}
	...
	for i, line := range lines {
		...
		if rules.GMatchLower(reKeychainAlways, line, lowered[i]) {
			matched := rules.GFindLower(reKeychainAlways, line, lowered[i])
			if len(matched) > 120 {
				matched = matched[:120] + "..."
			}
			findings = append(findings, rules.Finding{
...
func (r *SwiftUserDefaultsSensitive) Description() string {
	return "Detects Swift UserDefaults storing sensitive data (passwords, tokens, keys) which is stored unencrypted on the device."
}
func (r *SwiftUserDefaultsSensitive) DefaultSeverity() rules.Severity { return rules.High }
func (r *SwiftUserDefaultsSensitive) Languages() []rules.Language {
	return []rules.Language{rules.LangSwift}
}

func (r *SwiftUserDefaultsSensitive) Scan(ctx *rules.ScanContext) []rules.Finding {
	if !rules.GMatchFile(reUserDefaultsSet, ctx) {
		return nil
	}
	...
	for i, line := range lines {
		...
		if m := rules.GFindLower(reUserDefaultsSensKey, line, lowered[i]); m != "" {
			matched := m
			if len(matched) > 120 {
				matched = matched[:120] + "..."
			}
...
func (r *SwiftWKWebViewJS) Description() string {
	return "Detects Swift WKWebView with JavaScript enabled but without content rule lists or navigation delegation for URL restriction."
}
...
func (r *SwiftWKWebViewJS) Scan(ctx *rules.ScanContext) []rules.Finding {
	if !rules.GMatchFile(reWKPrefsJSEnabled, ctx) {
		return nil
	}

	// Skip if content rules or navigation delegate is set
	if rules.GMatchFile(reWKContentRules, ctx) && rules.GMatchFile(reWKNavDelegate, ctx) {
		return nil
	}
	...
	for i, line := range lines {
		...
		if rules.GMatchLower(reWKPrefsJSEnabled, line, lowered[i]) {
			matched := rules.GFindLower(reWKPrefsJSEnabled, line, lowered[i])
			if len(matched) > 120 {
				matched = matched[:120] + "..."
			}
			findings = append(findings, rules.Finding{
...
func (r *SwiftHardcodedKey) Description() string {
	return "Detects Swift hardcoded encryption keys, IVs, or nonces as byte arrays or string literals."
}
...
func (r *SwiftHardcodedKey) Scan(ctx *rules.ScanContext) []rules.Finding {
	...
	for i, line := range lines {
		...
		if m := rules.GFindLower(reHardcodedKey, line, lowered[i]); m != "" {
			matched = m
		} else if m := rules.GFindLower(reHardcodedKeyStr, line, lowered[i]); m != "" {
			matched = m
		} else if m := rules.GFindLower(reHardcodedKeyData, line, lowered[i]); m != "" {
			matched = m
		}
...
func (r *SwiftSQLPredicateInterp) Description() string {
	return "Detects Swift string interpolation in SQL queries or NSPredicate format strings, enabling injection attacks."
}
func (r *SwiftSQLPredicateInterp) DefaultSeverity() rules.Severity { return rules.High }
func (r *SwiftSQLPredicateInterp) Languages() []rules.Language {
	return []rules.Language{rules.LangSwift}
}
...
func (r *SwiftSQLPredicateInterp) Scan(ctx *rules.ScanContext) []rules.Finding {
	...
	for i, line := range lines {
		...
		if m := rules.GFindLower(rePredicateInterp, line, lowered[i]); m != "" {
			matched = m
		...
		} else if m := rules.GFindLower(rePredicateConcat, line, lowered[i]); m != "" {
			matched = m
		...
		} else if m := rules.GFindLower(reSQLStringInterp, line, lowered[i]); m != "" {
			matched = m
...
func (r *SwiftNSCodingDeser) Description() string {
	return "Detects Swift NSKeyedUnarchiver.unarchiveObject (deprecated, insecure) without NSSecureCoding, enabling deserialization attacks."
}
...
func (r *SwiftNSCodingDeser) Scan(ctx *rules.ScanContext) []rules.Finding {
	...
	if rules.GMatchFile(reNSSecureCoding, ctx) {
		return nil
	}
	...
	for i, line := range lines {
		...
		if m := rules.GFindLower(reNSKeyedUnarchiver, line, lowered[i]); m != "" {
			matched = m
		} else if m := rules.GFindLower(reNSKeyedUnarchiverData, line, lowered[i]); m != "" {
			matched = m
		}
...
func (r *SwiftATSDisabled) Description() string {
	return "Detects App Transport Security (ATS) disabled via NSAllowsArbitraryLoads in Info.plist, allowing plaintext HTTP connections."
}
...
func (r *SwiftATSDisabled) Scan(ctx *rules.ScanContext) []rules.Finding {
	if !rules.GMatchFile(reATSInfoPlist, ctx) {
		return nil
	}
	...
	for i, line := range lines {
		...
		if rules.GMatchLower(reATSDisabled, line, lowered[i]) {
