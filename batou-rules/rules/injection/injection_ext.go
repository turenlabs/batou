...
//
// Tightening note (2026-04-26): the previous reLogForging matched bare
// `body|query|params|args|input|user_input` anywhere after a logger
// call — including ENGLISH WORDS in log message strings like
// `"unable to read discovery response body"`. 36 FPs in ocis.
// Tightened to require actual property-access shape — `params.X`,
// `query.X`, `body.X`, `args.X`, `args[`, etc. Also added a skip for
// structured-logger field calls (`.Str()`, `.Err()`, `.WithField()`)
// which escape values and aren't vulnerable to log injection.
var (
	...
	// Structured-logger chained field methods (zerolog, zap, slog,
	// logrus) — these escape values and prevent log injection.
	reLogStructuredFields = regexp.MustCompile(`\.(?:Str|String|Int(?:64|32|16|8)?|Uint(?:64|32|16|8)?|Float(?:64|32)?|Bool|Time|Dur|Err|Bytes|Hex|Interface|Any|Stringer|RawJSON|Fields|WithField|WithFields|With)\s*\(`)
)
...
//
// Tightening note (2026-04-26): reFormulaWrite previously had an
// optional `(?:Row|Cell|Field)?` suffix, so bare `add(ctx, user)`,
// `append(users, user)`, `write(buf)` matched whenever the line
// contained any of `user|data|input|param`. Produced 84 FPs in
// owncloud/ocis (`Add(ctx, user)` in service constructors etc.).
// Made the spreadsheet-specific suffix required — `addRow`, `addCell`,
// `appendField`, `writeRow` etc. only.
var (
...
func (r *LDAPInjectionBroad) Scan(ctx *rules.ScanContext) []rules.Finding {
	...
	for i, line := range lines {
		...
		for _, p := range pats {
			if loc := rules.GFindIndexLower(p, line, lowered[i]); loc != nil {
...
func (r *LogInjection) Scan(ctx *rules.ScanContext) []rules.Finding {
	...
	for i, line := range lines {
		...
		// Skip structured-logger field calls — zerolog/zap/slog/logrus
		// `.Str()`, `.Err()`, `.WithField()` etc. escape values, so log
		// injection is not possible. Common in Go (and growing in JS).
		if rules.GMatchLower(reLogStructuredFields, line, lowered[i]) {
			continue
		}
		for _, p := range pats {
			if loc := rules.GFindIndexLower(p, line, lowered[i]); loc != nil {
...
func (r *RegExpInjection) Scan(ctx *rules.ScanContext) []rules.Finding {
	...
	for i, line := range lines {
		...
		if loc := rules.GFindIndexLower(reRegexNew, line, lowered[i]); loc != nil {
			findings = append(findings, rules.Finding{
				RuleID: r.ID(), Severity: r.DefaultSeverity(), SeverityLabel: r.DefaultSeverity().String(),
		...
		} else if loc := rules.GFindIndexLower(reRegexVar, line, lowered[i]); loc != nil {
...
func (r *MongoOperatorInjection) Scan(ctx *rules.ScanContext) []rules.Finding {
	...
	if rules.GMatchFile(reMongoNoSanitize, ctx) {
		return nil
	}
	var findings []rules.Finding
	lines := ctx.SplitLines()
	...
	for i, line := range lines {
		...
		if loc := rules.GFindIndexLower(reMongoReqDirect, line, lowered[i]); loc != nil {
			findings = append(findings, rules.Finding{
				RuleID: r.ID(), Severity: r.DefaultSeverity(), SeverityLabel: r.DefaultSeverity().String(),
