...
var (
	...
	// JS/TS concurrency markers — browser/Node single-threaded code only has
	// a data race when it shares state across Workers / SharedArrayBuffer /
	// the cluster module. Without these (or server-side persistence), a
	// `count += 1` is a UI counter, not a race. Frontend upload-progress
	// counters (`successCount += 1` in .vue/.ts) generated ~9 FPs in
	// owncloud/web.
	reRaceJSConcurrency = regexp.MustCompile(`(?i)(?:\bnew\s+Worker\b|\bnew\s+SharedWorker\b|SharedArrayBuffer|\bAtomics\.|worker_threads|require\s*\(\s*['"]cluster['"]|from\s+['"]cluster['"]|\bcluster\.fork\b|navigator\.serviceWorker|MessageChannel)`)
)
...
func hasNearbyPattern(lines []string, idx, before, after int, re *regexp.Regexp) bool {
	...
	for _, l := range lines[start:end] {
		...
		// Finding-identical — the gate never returns false on a real match.
...
func (r *CheckThenAct) Languages() []rules.Language {
	// Restricted to languages with real shared-memory concurrency. JS/TS
	// are single-threaded (event loop) — `if (cache.has(k)) cache.get(k)`
	// patterns there cannot race within the same execution context, and
	// firing the rule produced 22 FPs on owncloud/web alone. Web Workers
	// do introduce parallelism but communicate via postMessage, not
	// shared map operations, so the pattern this rule looks for still
	// doesn't apply. Ruby's GVL serializes most check-then-act sequences
	// on MRI; dropped pending an MRI-vs-JRuby gate.
	return []rules.Language{rules.LangGo, rules.LangJava, rules.LangPython, rules.LangCSharp}
}

// reCheckThenActAllowlistTuple matches `if x [not] in (...)` / `if x [not] in [...]`
// — Python-style allowlist validation against a literal tuple, list, or set.
// These are input-validation guards, not check-then-act on a shared container,
// so they should never trigger BATOU-RACE-002. The pattern intentionally
// requires the operand to be a parenthesised / bracketed / braced literal to
// avoid suppressing real `if key in cache: cache[key]` shapes.
var reCheckThenActAllowlistTuple = regexp.MustCompile(`if\s+.*\b(?:not\s+)?in\s*[\(\[\{]`)
...
func (r *CheckThenAct) Scan(ctx *rules.ScanContext) []rules.Finding {
	...
	for i, line := range lines {
		...
		// Skip Python URL/scheme/value allowlist validation: `if x in (a, b)`
		// and `if x not in {...}` are guard patterns, not check-then-act on a
		// shared mutable container. Triggered by CVE-2023-24329 / CVE-2023-32681
		// safe fixtures where the bare `not in (...)` form was flagged as a
		// race condition.
		if rules.GMatch(reCheckThenActAllowlistTuple, line) {
			continue
		}
...
func (r *BalanceRace) Scan(ctx *rules.ScanContext) []rules.Finding {
	// For JS/TS (which includes .vue SFCs), a non-atomic counter update is
	// only a race if state is shared across true concurrency primitives
	// (Workers, SharedArrayBuffer, cluster) OR this is server-side code where
	// concurrent HTTP requests can touch the same persistent row. Plain
	// frontend code (`successCount += 1` in an upload-progress component) is
	// single-threaded and not a race condition.
	if ctx.Language == rules.LangJavaScript || ctx.Language == rules.LangTypeScript {
		if !rules.GMatchFile(reRaceJSConcurrency, ctx) && !rules.IsServerSideCode(ctx) {
			return nil
		}
	}
...
func (r *DoubleCheckedLocking) Scan(ctx *rules.ScanContext) []rules.Finding {
	...
	for i, line := range lines {
		...
		if rules.GMatch(reDoubleCheckLock, line) {
			...
			if hasNearbyPattern(lines, i, 0, 5, reSyncBlock) {
				...
				for j := innerStart; j < innerEnd; j++ {
					if rules.GMatch(reDoubleCheckInner, lines[j]) {
...
func (r *SignalHandlerRace) Scan(ctx *rules.ScanContext) []rules.Finding {
	...
	for i, line := range lines {
		...
		if rules.GMatch(reSignalHandler, line) {
		...
		if inSignalHandler {
			...
			if m := rules.GFind(reNonReentrantFunc, line); m != "" {
				findings = append(findings, rules.Finding{
					RuleID:        r.ID(),
					Severity:      r.DefaultSeverity(),
					SeverityLabel: r.DefaultSeverity().String(),
...
func (r *SharedMutableState) Scan(ctx *rules.ScanContext) []rules.Finding {
	...
	case rules.LangGo:
		for i, line := range lines {
			...
			if m := rules.GFind(reGoGoroutineWrite, line); m != "" {
	...
	case rules.LangJava:
		for i, line := range lines {
			...
			if m := rules.GFind(reJavaThreadStart, line); m != "" {
...
func (r *NonAtomicRMW) Scan(ctx *rules.ScanContext) []rules.Finding {
	...
	for _, line := range lines {
		if rules.GMatch(reGoGoroutineWrite, line) || rules.GMatch(reJavaThreadStart, line) || rules.GMatch(reSyncBlock, line) {
			hasConcurrency = true
			break
		}
	}
...
func (r *LazyInitRace) Scan(ctx *rules.ScanContext) []rules.Finding {
	...
	case rules.LangJava:
		for i, line := range lines {
			...
			if rules.GMatch(reLazyInitJava, line) {
	...
	case rules.LangCSharp:
		for i, line := range lines {
			...
			if rules.GMatch(reLazyInitCS, line) {
