...
The repo is a two-module workspace: regex rules are generated into
batou-rules/rules/, taint catalog entries into batou-core/taint/languages/,
and blank imports for new rule categories into batou-core/cmd/batou/main.go.
The repo root is auto-detected from the script location, so the tool works
from any CWD.

Usage:
    python3 batou-rules/tools/generate_rules.py rules.yaml              # generate + verify
    python3 batou-rules/tools/generate_rules.py --dry-run rules.yaml    # preview only
    python3 batou-rules/tools/generate_rules.py --no-verify rules.yaml  # skip go build
"""
...
def find_repo_root() -> Path:
    ...
    batou-rules/) — go.work is used as a fallback marker.
    """
...
MAIN_GO = PROJECT_ROOT / "batou-core" / "cmd" / "batou" / "main.go"

# Go import paths for the two modules (see go.work).
RULES_IMPORT = "github.com/turenlabs/batou-rules/rules"
TAINT_IMPORT = "github.com/turenlabs/batou-core/taint"
...
def _match_brace(content: str, brace_start: int) -> int:
    """Find the position of the '}' matching the '{' at brace_start.

    Skips string literals (backtick raw strings, double-quoted strings, rune
    literals) and comments, since catalog regex patterns routinely contain
    unbalanced braces (e.g. `\\{` or `[({]`). Returns -1 if unmatched.
    """
    depth = 0
    i = brace_start
    n = len(content)
...
def _insert_into_slice(file_path: Path, entries_text: str) -> str:
    ...
    # Find the opening brace of the slice literal, then its matching close
    # (string/comment-aware — patterns contain unbalanced braces).
    brace_start = content.index("{", return_match.start())
    insert_pos = _match_brace(content, brace_start)
...
def update_main_imports(categories: set[str], dry_run: bool) -> bool:
    """Add blank imports to batou-core/cmd/batou/main.go for new rule categories.
    ...
    for cat in sorted(categories):
        import_path = f'_ "{RULES_IMPORT}/{cat}"'
...
def run_go_build() -> bool:
    """Run go build ./... in each workspace module to verify generated code compiles.

    The repo root is a go.work workspace without its own go.mod, so ./...
    must be expanded per module (batou-rules first: batou-core depends on it).
    """
    for module_dir in (PROJECT_ROOT / "batou-rules", PROJECT_ROOT / "batou-core"):
        print(f"Running go build ./... in {module_dir.name} to verify...")
        try:
            result = subprocess.run(
                ["go", "build", "./..."],
            ...
            return True
        if result.returncode != 0:
            print(f"go build FAILED in {module_dir.name}:\n{result.stderr}", file=sys.stderr)
            return False
...
def main():
    ...
    if new_categories:
        ...
        if updated and not args.dry_run:
            all_modified_files.append(str(MAIN_GO))
