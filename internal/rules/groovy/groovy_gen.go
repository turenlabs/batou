...
// GVY-017
var (
	reJenkinsFileParam = regexp.MustCompile(`\b(?:readFile|writeFile)\s*\([^)]*\$\{.*\b(?:params|input|env)\b`)
)
...
func (r *JenkinsFileTraversal) Description() string {
	return "Detects Jenkins readFile/writeFile steps with user-controlled paths."
}
func (r *JenkinsFileTraversal) Languages() []rules.Language {
...
func (r *GrabDependency) Description() string {
	return "Detects @Grab annotations pulling dependencies from potentially untrusted repositories."
}
...
func (r *JenkinsLoadInjection) Description() string {
	return "Detects Jenkins load step with user-controlled script paths."
}
func (r *JenkinsLoadInjection) Languages() []rules.Language {
	return []rules.Language{rules.LangGroovy}
}
...
func (r *GrailsRenderXSS) Description() string {
	return "Detects Grails render with text parameter containing user input."
}
...
func (r *ValueSpELInjection) Description() string {
	return "Detects @Value annotations mixing property placeholders with SpEL expressions."
}
...
func (r *JsonSlurperURL) Description() string {
	return "Detects JsonSlurper.parse() with URL arguments from untrusted sources."
}
...
func (r *JenkinsHttpRequestSSRF) Description() string {
	return "Detects Jenkins httpRequest step with user-controlled URLs."
}
func (r *JenkinsHttpRequestSSRF) Languages() []rules.Language {
	return []rules.Language{rules.LangGroovy}
}
...
func (r *GORMExecuteUpdateInjection) Description() string {
	return "Detects GORM executeUpdate with GString interpolation enabling HQL injection."
}
func (r *GORMExecuteUpdateInjection) Languages() []rules.Language {
	return []rules.Language{rules.LangGroovy}
}
...
func (r *PipelineInputNoTimeout) Description() string {
	return "Detects Jenkins pipeline input steps without timeout wrapper."
}
func (r *PipelineInputNoTimeout) Languages() []rules.Language {
	return []rules.Language{rules.LangGroovy}
}
func (r *PipelineInputNoTimeout) Scan(ctx *rules.ScanContext) []rules.Finding {
	var findings []rules.Finding
	if !rePipelineInput.MatchString(ctx.Content) || strings.Contains(ctx.Content, "timeout(") {
		return nil
	}
	for i, line := range strings.Split(ctx.Content, "\n") {
		if isComment(line) {
...
func (r *NonCPSSecurityOps) Description() string {
	return "Detects @NonCPS methods performing security-sensitive operations."
}
...
func (r *NonCPSSecurityOps) Scan(ctx *rules.ScanContext) []rules.Finding {
	var findings []rules.Finding
	if !reNonCPS.MatchString(ctx.Content) {
		return nil
	}
