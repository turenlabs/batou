...
// ZIG-011: @intCast with external/untrusted data context
var (
	reIntCast    = regexp.MustCompile(`@intCast\s*\(`)
	reIntCastCtx = regexp.MustCompile(`(?i)(?:parse|read|recv|input|request|header|socket|arg|extern|user|untrusted|from_bytes|readInt)`)
)
...
func (r *MemcpyOverflow) Description() string {
	return "Detects @memcpy usage which can cause buffer overflow if length is not validated."
}
...
func (r *CSystemInjection) Description() string {
	return "Detects calls to std.c.system() or std.c.popen() which execute shell commands."
}
...
func (r *IntCastOverflow) Description() string {
	return "Detects @intCast which can panic on narrowing casts if the value exceeds the target type range."
}
...
func (r *IntCastOverflow) Scan(ctx *rules.ScanContext) []rules.Finding {
	var findings []rules.Finding
	if !reIntCastCtx.MatchString(ctx.Content) {
		return findings
	}
	...
	for i, line := range lines {
		...
		if reIntCast.MatchString(line) && reIntCastCtx.MatchString(line) {
...
func (r *AllocatorUseAfterFree) DefaultSeverity() rules.Severity { return rules.High }
func (r *AllocatorUseAfterFree) Description() string {
	return "Detects allocator.free() or .deinit() patterns that may lead to use-after-free."
}
...
func (r *MissingErrdefer) Description() string {
	return "Detects allocations with try that lack a corresponding errdefer for cleanup on error paths."
}
...
func (r *MissingErrdefer) Scan(ctx *rules.ScanContext) []rules.Finding {
	var findings []rules.Finding
	if !reTryAlloc.MatchString(ctx.Content) {
		return nil
	}
	if reErrdefer.MatchString(ctx.Content) {
		return nil
	}
	for i, line := range strings.Split(ctx.Content, "\n") {
		if isCommentLine(line) {
...
func (r *EmbedFileSensitive) Description() string {
	return "Detects @embedFile embedding files with sensitive names (keys, secrets, certs) into the binary."
}
...
func (r *StdNetNoTLS) Description() string {
	return "Detects std.net.Stream usage without TLS context, transmitting data in plaintext."
}
...
func (r *StdNetNoTLS) Scan(ctx *rules.ScanContext) []rules.Finding {
	var findings []rules.Finding
	if !reStdNetStream.MatchString(ctx.Content) {
		return nil
	}
	if strings.Contains(ctx.Content, "tls") || strings.Contains(ctx.Content, "TLS") || strings.Contains(ctx.Content, "ssl") {
		return nil
	}
	for i, line := range strings.Split(ctx.Content, "\n") {
		if isCommentLine(line) {
...
func (r *SentinelSliceBypass) Description() string {
	return "Detects @ptrCast on .ptr field which bypasses sentinel-terminated slice safety guarantees."
}
...
func (r *CStringInterop) Description() string {
	return "Detects Zig slice .ptr passed to C functions that may expect null-terminated strings."
}
...
func (r *CStringInterop) Scan(ctx *rules.ScanContext) []rules.Finding {
	var findings []rules.Finding
	if !strings.Contains(ctx.Content, "@cImport") && !strings.Contains(ctx.Content, "std.c.") {
		return nil
	}
	for i, line := range strings.Split(ctx.Content, "\n") {
		if isCommentLine(line) {
...
func (r *ZigFormatString) Description() string {
	return "Detects std.fmt.format or std.log calls that may use runtime data in format position."
}
