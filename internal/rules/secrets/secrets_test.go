...
// --- BATOU-SEC-001: Hardcoded Password ---
...
func TestSEC001_Fixture_Go(t *testing.T) {
	...
	result := testutil.ScanContent(t, "/app/config.go", content)
	hasSEC := testutil.HasFinding(result, "BATOU-SEC-001") || testutil.HasFinding(result, "BATOU-SEC-005")
	if !hasSEC {
...
func TestSEC001_Fixture_JS(t *testing.T) {
	...
	result := testutil.ScanContent(t, "/app/config.ts", content)
	hasSEC := testutil.HasFinding(result, "BATOU-SEC-001") ||
		testutil.HasFinding(result, "BATOU-SEC-002") ||
		testutil.HasFinding(result, "BATOU-SEC-005")
	if !hasSEC {
...
func TestSEC002_AWSAccessKey(t *testing.T) {
	...
	result := testutil.ScanContent(t, "/app/config.py", content)
	testutil.MustFindRule(t, result, "BATOU-SEC-002")
}
...
func TestSEC002_GitHubToken(t *testing.T) {
	...
	testutil.MustFindAnyRule(t, result, "BATOU-SEC-001", "BATOU-SEC-002")
}
...
-----END RSA PRIVATE KEY-----"""`
	...
	testutil.MustFindRule(t, result, "BATOU-SEC-003")
}
...
// --- BATOU-SEC-004: Connection String ---
...
func TestSEC004_PostgresURI(t *testing.T) {
	...
	testutil.MustFindRule(t, result, "BATOU-SEC-004")
}
...
func TestSEC004_Safe_Placeholder(t *testing.T) {
	...
	result := testutil.ScanContent(t, "/app/db.go", content)
	testutil.MustNotFindRule(t, result, "BATOU-SEC-004")
}
...
func TestSEC005_JWTSign(t *testing.T) {
	...
	testutil.MustFindRule(t, result, "BATOU-SEC-005")
}
...
func TestSEC005_Fixture_JWT(t *testing.T) {
	...
	result := testutil.ScanContent(t, "/app/auth.ts", content)
	hasSEC := testutil.HasFinding(result, "BATOU-SEC-005") || testutil.HasFinding(result, "BATOU-SEC-001")
	if !hasSEC {
		t.Errorf("expected secret/JWT finding in jwt_none_algo.ts, got: %v", testutil.FindingRuleIDs(result))
	}
}

// --- BATOU-SEC-006: Environment Leak ---
...
API_SECRET_KEY=abc123def456`
	...
	testutil.MustFindRule(t, result, "BATOU-SEC-006")
}
