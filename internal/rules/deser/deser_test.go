...
db = shelve.open(user_path)`
	...
	testutil.MustFindRule(t, result, "BATOU-DESER-001")
}
...
Object obj = kryo.readObject(input, MyClass.class);`
	...
	testutil.MustFindAnyRule(t, result, "BATOU-DESER-001", "BATOU-DESER-005")
}
...
Object data = yaml.load(userInput);`
	result := testutil.ScanContent(t, "/app/Config.java", content)
	testutil.MustNotFindRule(t, result, "BATOU-DESER-001")
}
...
eval(user_code)`
	...
	testutil.MustFindRule(t, result, "BATOU-DESER-002")
}
...
func TestDESER002_RubyEvalStringLiteral_Safe(t *testing.T) {
	...
	testutil.MustNotFindRule(t, result, "BATOU-DESER-002")
}
...
$result = preg_replace('/test/e', $input, "test string");`
	...
	testutil.MustFindRule(t, result, "BATOU-DESER-003")
}
...
$result = preg_replace('/test/i', 'replacement', $input);`
	result := testutil.ScanContent(t, "/app/handler.php", content)
	testutil.MustNotFindRule(t, result, "BATOU-DESER-003")
}
...
// ---------------------------------------------------------------------------
// BATOU-DESER-004: JS/TS setTimeout/setInterval with String
// ---------------------------------------------------------------------------
...
setTimeout('alert(1)', 1000);`
	...
	testutil.MustFindRule(t, result, "BATOU-DESER-004")
}
...
func TestDESER004_SetTimeoutFunction_Safe(t *testing.T) {
	...
	result := testutil.ScanContent(t, "/app/handler.ts", content)
	testutil.MustNotFindRule(t, result, "BATOU-DESER-004")
}
...
func TestDESER001_Fixture_PHP_Deserialization(t *testing.T) {
	...
	hasDeser := testutil.HasFinding(result, "BATOU-GEN-002") || testutil.HasFinding(result, "BATOU-DESER-001")
	if !hasDeser {
...
func TestDESER002_Fixture_Ruby_Deserialization(t *testing.T) {
	...
	result := testutil.ScanContent(t, "/app/handler.rb", content)
	hasDeser := testutil.HasFinding(result, "BATOU-GEN-002") ||
		testutil.HasFinding(result, "BATOU-DESER-002") ||
		testutil.HasFinding(result, "BATOU-GEN-011")
	if !hasDeser {
		t.Logf("findings: %v", testutil.FindingRuleIDs(result))
	}
}
